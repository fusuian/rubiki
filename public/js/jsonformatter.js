...
Json.Formatter.prototype = {
	...
	initialize : function(obj) {
		...
		if( type == "undefined" ) {
			...
			var isInstruction = (typeof(obj[0]) == "string" && obj[0] != "YARVInstructionSequence/SimpleDataFormat");
			
			buffer.push( "[" );
			buffer.push( (hasProp && !isInstruction)? "\r\n" : "" );
			// 要素を再帰処理
			for(var index = 0; index < obj.length; index++) {
				var v = obj[ index ];
				if( v != null ) {
					if (isInstruction) {
						buffer.push( new Json.Formatter( v, null, 0 ).value );					
					} else {
			...
			buffer.push( (hasProp && !isInstruction)? current_indent : "" );
			buffer.push( "]" );
